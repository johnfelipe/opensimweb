...
$getsize = $qls->Security->make_safe($_GET['size']);
...
if(isset($getsize) && $ALLOW_ZOOM==TRUE){
  if($getsize == 32 || $getsize == 64 || $getsize == 128 || $getsize == 192 or $getsize == 256)
  {
    $size=$getsize;
  }
} else {
...
<?php
$tbl1 = $osw->SQL->query("SELECT regionName,locX,locY,owner_uuid,uuid,regionMapTexture FROM `{$osw->config['robust_db']}`.regions Order by locX");
while(list($regionName,$locX,$locY,$owner,$ruuid,$mapuuid) = $osw->SQL->fetch_array($tbl1)){
